...
class AgentContext {
   ...
   * @param {number} waitMs - 等待超时时间（毫秒）
   */
  async _acquireLock(lockPath, waitMs = 5000) {
    ...
    while (pos < total) {
      ...
      switch (state) {
        ...
        case 'BODY': {
          ...
          // 解析 tags（增强容错：处理空数组、格式异常）
          let tags = [];
          if (meta.tags) {
            let rawTags = meta.tags;
            
            // 如果 tags 已经是数组，直接使用
            if (Array.isArray(rawTags)) {
              tags = rawTags.filter(t => t && t.trim().length > 0);
            } 
            // 如果 tags 是字符串，解析逗号分隔的值
            else if (typeof rawTags === 'string') {
              const trimmed = rawTags.trim();
              if (trimmed !== '' && trimmed !== '[]') {
                try {
                  tags = trimmed.split(',').map(t => t.trim()).filter(t => t && t.length > 0);
                } catch (e) {
                  tags = [];
                }
              }
            }
            // 其他类型（null, undefined, 对象等）→ 空数组
            else {
              tags = [];
            }
          }
  ...
  _saveVectorIndex(index) {
    ...
    // 使用原子写入，避免并发写入损坏文件
    this._atomicWrite(indexPath, JSON.stringify(index, null, 2));
  }
  ...
  /**
   * 压缩共享层记忆：合并稀疏条目，生成摘要
   * @param {number} maxEntriesPerLayer - 每层最大条目数（默认50）
   */
  async compact(maxEntriesPerLayer = 50) {
    const layers = ['mentalModels', 'observations', 'worldFacts'];
    const results = {};

    for (const layer of layers) {
      const entries = await this.readShared(layer);
      if (entries.length <= maxEntriesPerLayer) {
        results[layer] = { kept: entries.length, action: 'skip' };
        continue;
      }
      ...
      const highConf = entries
        ...
        .slice(0, maxEntriesPerLayer);

      // 生成摘要压缩
      const summary = this._generateSummary(entries, maxEntriesPerLayer);

      // 覆盖写入压缩后的内容
      const compactContent = this._entriesToMarkdown(layer, highConf, summary);
      const filePath = this.getMemoryPath(layer, 'shared');
      this._atomicWrite(filePath, compactContent);

      results[layer] = {
        kept: highConf.length,
        compressed: entries.length - highConf.length,
        action: 'compact'
      };
    }
   ...
   * 生成记忆摘要
   */
  _generateSummary(entries, maxLen) {
    const lines = entries.map(e => e.content.substring(0, 100));
    return `## 压缩摘要（共${entries.length}条，保留${maxLen}条高置信度）\n` +
      `来源：${entries.map(e => e.agent || '?').join(', ')}\n`;
  }

  /**
   * 将 entries 转回 markdown 格式
   */
  _entriesToMarkdown(layer, entries, summary) {
    const parts = [];
    for (const e of entries) {
      const meta = [
        '---',
        `source: ${e.agent || 'unknown'}`,
        `layer: ${layer}`,
